...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("leader is %d", leader1)

	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	DPrintf("Raft %d disconnect", leader1)

	leader2 := cfg.checkOneLeader()
	DPrintf("new leader is %d", leader2)

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	DPrintf("Raft %d connect 此时应该新leader应该保持不变", leader1)
	leader3 := cfg.checkOneLeader()

	DPrintf("leader is %d", leader3)

	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	DPrintf("Raft %d %d", leader2, (leader2+1)%servers)
	cfg.disconnect((leader2 + 1) % servers)
	// cfg.disconnect((leader2 + 2) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	// DPrintf("HHHHHHHHHHHHHH")
